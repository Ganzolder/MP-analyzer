...
/**
 * Внутренняя функция расчёта - выполняет расчёт для заданной цены
 * Возвращает результат без обёртки NextResponse
 */
  ...
  try {
    ...
    const findShippingCost = async (deliveryMethod: string): Promise<{ cost: number; tariffDetails: string }> => {
      if (priceBand === "up_to_300") {
        ...
        if (!tariff) return { cost: 0, tariffDetails: "Тариф не найден" };
        return { cost: tariff.basePrice, tariffDetails: `Фикс. ${tariff.basePrice} ₽ (объём ${volumeLiters.toFixed(3)} л)` };
      }
      ...
      if (roundedVolumeLiters <= 1) {
        ...
        const volumeCost = volumeOver3L * tariff3To190L.basePrice;
        cost = fixedCost + volumeCost;
        details.push(`Фикс. ${fixedCost} ₽ (2-3л) + ${volumeOver3L} л × ${tariff3To190L.basePrice} ₽/л`);
      } else if (roundedVolumeLiters <= 1000) {
        ...
        const volumeCostOver190 = volumeOver190L * tariff190To1000L.basePrice;
        cost = fixedCost + volumeCost3To190 + volumeCostOver190;
        details.push(`Фикс. ${fixedCost} ₽ (2-3л) + ${volume3To190L} л × ${tariff3To190L.basePrice} ₽/л + ${volumeOver190L} л × ${tariff190To1000L.basePrice} ₽/л`);
      } else {
      ...
      cost = Math.round(cost * 100) / 100;
      return { cost, tariffDetails: details.join(" | ") };
    };
...
async function calculatePriceByMargin(params: {
  ...
  let price = totalCost * (1 + targetMargin / 100) * 1.3;
  const maxIterations = 50;
  const tolerance = 0.1; // Точность 0.1%
  ...
  for (let i = 0; i < maxIterations; i++) {
    try {
      ...
      if (!result.success) {
        return NextResponse.json(
          { success: false, error: result.error || "Ошибка при расчёте" },
          { status: 500 }
        );
      }

      const totalFees = result.data![fulfillmentType].totalFees;
      ...
      if (actualMargin < targetMargin) {
        price = price * (1 + (targetMargin - actualMargin) / 100 + 0.03);
      } else {
        price = price * (1 - (actualMargin - targetMargin) / 100 - 0.01);
      }
