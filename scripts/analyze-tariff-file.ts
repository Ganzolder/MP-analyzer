 ...
 */
async function analyzeTariffFile(filePath: string) {
  try {
    ...
    console.log("\n🔍 Анализ типов данных:");
    const columnTypes: Record<number, Set<string>> = {};
    ...
    const result = {
      ...
      headers,
      sampleRows: data.slice(1, 6).map((row, index) => {
        const obj: any = {};
        headers.forEach((header, colIndex) => {
...
if (!filePath) {
  console.log("Использование: npx tsx scripts/analyze-tariff-file.ts <путь-к-файлу>");
  console.log("Пример: npx tsx scripts/analyze-tariff-file.ts \"База/тарифы.xlsx\"");
  process.exit(1);
}
