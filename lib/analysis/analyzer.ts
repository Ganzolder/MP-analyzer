...
export class OzonReportAnalyzer {
  ...
  private recommendationGenerator: RecommendationGenerator | null = null;
  private topProductsHelper = new TopProductsHelper();
  
  private getRecommendationGenerator(): RecommendationGenerator {
    if (!this.recommendationGenerator) {
      this.recommendationGenerator = new RecommendationGenerator();
    }
    return this.recommendationGenerator;
  }
  ...
  async analyze(
    ...
    // 2. Агрегация заказов
    const orders = this.orderAggregator.aggregateOrders(
      parseResult.chargeRows,
    ...
    // 9. Проблемные зоны и рекомендации (без AI - AI вызывается отдельно по требованию)
    const problemAreas = this.problemIdentifier.identifyProblemAreas(orders, productMetrics, costBreakdown);
    const recommendations = await this.getRecommendationGenerator().generateRecommendations(
      summary, 
      costBreakdown, 
      problemAreas,
      topProducts,
      false // AI отключен - будет вызываться отдельно
    );
